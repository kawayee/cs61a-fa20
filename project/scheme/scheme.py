...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            ...
            macro_expr = operator.apply_macro(rest, env)
...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    curr, result = expressions, None
    while(curr.rest != nil):
        result = scheme_eval(curr.first, env)
        curr = curr.rest
    result = scheme_eval(curr.first, env, True) # PROBLEM 19
    # print(f'eval_all({expressions}, {env}, {tail})-->{result}')
    return result
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        for _ in range(len(formals)):
            ...
            cur_formal, cur_val = cur_formal.rest, cur_val.rest
        return new_frame
        # END PROBLEM 10
...
def do_and_form(expressions, env):
    ...
    while(curr != nil):
        result = scheme_eval(curr.first, env, True if curr.rest is nil else False)
...
def make_let_frame(bindings, env):
    ...
    while(cur_binding != nil):
        ...
        values = Pair(scheme_eval(cur_binding.first.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        formals = target.rest
        body = expressions.rest
        env.define(name, do_macro_form(Pair(formals, body), env))
        return name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            # print(f'new_eval({expr}, {env}, {tail})')
            # 当optimized_eval接受非原子的tail-call时，它立即返回Thunk实例，而不是继续递归scheme_eval，
            # 执行顺序是先返回Thunk实例，再计算Thunk实例中的expr，相当于回收之前的递归深度/空间。
            # define-->lambda, cond, let, begin 涉及 eval_all。
            # if 涉及 do_if_form。
        ...
        while isinstance(result, Thunk):
            # print(f'old_eval({result.expr}, {result.env}, {tail})')
