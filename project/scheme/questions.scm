  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))  ; lambda or define
               (params (cadr expr)) ; formals or name&formals
               (body   (cddr expr))); body or body
           ; BEGIN PROBLEM EC
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           ; BEGIN PROBLEM EC
           (define params-names (car (zip values)))
           (define params-values (map let-to-lambda (cadr (zip values))))
           (define lambda-body (map let-to-lambda body))
           (cons (append (list 'lambda params-names) lambda-body) params-values)
           ; END PROBLEM EC
           ))
        (else
         ; BEGIN PROBLEM EC
