...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if(who == 0):
            cur_score, cur_running_high = score0, running_high
        else:
            cur_score, cur_running_high = score1, running_high
        if((cur_score - last_score) > running_high):
            cur_running_high = cur_score - last_score
            print(f"{cur_running_high} point(s)! The most yet for Player {who}")
        return announce_highest(who, cur_score, cur_running_high)
    return say
    # END PROBLEM 7
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    1.优先获胜
    2.优先获取额外回合
    3.若优势or均势,取max_scoring_num_rolls,若劣势,取extra_turn_strategy
    *** YOUR DESCRIPTION HERE ***
    """
    bacon = free_bacon(opponent_score) # 3 <= bacon <= 12
    cutoff = 8
    num_rolls = 6 # max_scoring_num_rolls
    if score + bacon >= 100 or extra_turn(bacon + score, opponent_score):
        return 0
    elif 3 <= opponent_score - score <= 4:
        return 1
    elif opponent_score - score >= 30:
        return extra_turn_strategy(score, opponent_score, 10, 8)
    else:
