...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curplace, currange, curbees = self.place, 0, []
        while(currange < self.min_range and curplace != beehive):
            curplace = curplace.entrance
            currange += 1
        while(currange <= self.max_range and curplace != beehive):
            if(len(curplace.bees) != 0):
                curbees = curplace.bees
                break
            curplace = curplace.entrance
            currange += 1
        return rANTdom_else_none(curbees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        left_bees = self.place.bees[:]
        bee_amount = amount
        if(self.armor - amount <= 0):
            bee_amount += self.damage
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if(self.is_true_queen == False):
            ...
            while(curplace):
                if(curplace.ant and not curplace.ant.buffed):
                    curplace.ant.damage = curplace.ant.damage*2
                    curplace.ant.buffed = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 5
        curplace, distance = self.place, 0
        result = {}
        while(curplace != beehive):
            for bee in curplace.bees:
                result[bee] = distance
            if(curplace.ant):
                result[curplace.ant] = distance
            curplace, distance = curplace.entrance, distance + 1
        result.pop(self)
        return result
        # END Problem Optional 5
